...
一個「**做完了的專題**」長什麼樣——共同要求 10 條（[projects.md](../../projects.md) §3）全數達標的完整示範，兼作教師錄影腳本。
**本題不在指派清單內**：請模仿它的結構與完成度，不要抄它的程式（口試任指一段要能解釋）。
...
| §3 | 固定 seed 合成 14,000+ 列（長尾、學期節律、右偏逾期）＋庫存對帳 | 2 |
...
| §9 | 測試總驗收 11 項（含 4 個「應該失敗」） | 8 |
| §10–§11 | AI 使用說明（示範寫法）＋錄影備忘與觀察重點 | 9、10 |
